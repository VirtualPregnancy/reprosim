...
module testdrive
  ...
  interface check
    ...
    module procedure :: check_array_int
    module procedure :: check_array_int_int
    module procedure :: check_array_int_int_int
    module procedure :: check_array_sp
    module procedure :: check_array_dp
    module procedure :: check_array_dp_dp
  end interface check
...
contains
  ...
  subroutine check_array_int(error, actual, expected, message, more)
    ...
    rank_A = size(actual)
    rank_B = size(expected)
    
    if (rank_A == 0 .or. rank_B == 0) then
      call test_failed(error, "size of arrays not the same", &
        "size difference '"//to_string(rank_A)//"' and '"//to_string(rank_B)//"'.", more)
    end if
    
    if (rank_A .ne. rank_B) then
    end if
  
  end subroutine check_array_int
  
  subroutine check_array_int_int(error, actual, expected, message, more)
    ...
    integer :: i, j, rank_A1, rank_A2, rank_B1, rank_B2
    
    rank_A1 = size(actual, 1)
    rank_A2 = size(actual, 2)
    rank_B1 = size(expected, 1)
    rank_B2 = size(expected, 2)
    
    if (rank_A1 == 0 .or. rank_B1 == 0) then
      call test_failed(error, "one of the arrays is scalar", &
        "sizes are '"//to_string(rank_A1)//"' and '"//to_string(rank_B1)//"'.", more)
    end if
    
    if (rank_A1 .ne. rank_B1 .or. rank_A2 .ne. rank_B2) then
      call test_failed(error, "size of arrays not the same", &
        "size difference '"//to_string(rank_A1)//"' and '"//to_string(rank_B1)//"'.", more)
    end if
    
    do i = 1, rank_A1
      do j = 1, rank_A2
        if (expected(i, j) .ne. actual(i, j)) then
          call test_failed(error, "mismatch in array content", &
            "at index: ("//to_string(i)//", "//to_string(j)//	") the mismatch is: " &
            //to_string(expected(i, j))//" <--> "//to_string(actual(i, j))//".", more)
        end if
      end do
    end do
  
  end subroutine check_array_int_int
  
  subroutine check_array_int_int_int(error, actual, expected, message, more)
    ...
    integer :: rank_A1, rank_A2, rank_A3
    integer :: rank_B1, rank_B2, rank_B3
    integer :: i, j, k
    
    rank_A1 = size(actual, 1)
    rank_A2 = size(actual, 2)
    rank_A3 = size(actual, 3)
    rank_B1 = size(expected, 1)
    rank_B2 = size(expected, 2)
    rank_B3 = size(expected, 3)
    ...
    if (rank_A1 .ne. rank_B1 .or. rank_A2 .ne. rank_B2 .or. rank_A3 .ne. rank_B3) then
      call test_failed(error, "size of arrays not the same", &
        "size difference ('"//to_string(rank_A1)//", "//to_string(rank_A2)//", "//&
        to_string(rank_A3)//"') and ('"//to_string(rank_B1)//", "//to_string(rank_B2)//&
        ", "//to_string(rank_B3)//"').", more)
    end if
    
    do i = 1, rank_A1
      do j = 1, rank_A2
        do k = 1, rank_A3
          if (expected(i, j, k) .ne. actual(i, j, k)) then
            call test_failed(error, "mismatch in array content", &
              "at index: ("//to_string(i)//", "//to_string(j)//", "//to_string(k)//") the mismatch is: " &
              //to_string(expected(i, j, k))//" <--> "//to_string(actual(i, j, k))//".", more)
          end if
        end do
      end do
    end do
  ...
  end subroutine check_array_int_int_int
  
  subroutine check_array_sp(error, actual, expected, message, more, thr, rel)
    ...
    if (rank_A == 0 .or. rank_B == 0) then
      call test_failed(error, "one of the arrays is scalar", &
        "sizes are '"//to_string(rank_A)//"' and '"//to_string(rank_B)//"'.", more)
    end if
    ...
    do i = 1, rank_A
      ...
      if (allocated(error)) then
        error%message = error%message // skip // "array mismatch at element index "//trim(to_string(i))
        return
      end if
    end do
  
  end subroutine check_array_sp
  
  subroutine check_array_dp(error, actual, expected, message, more, thr, rel)
  ...
  end subroutine check_array_dp
  
  subroutine check_array_dp_dp(error, actual, expected, message, more, thr, rel)
    ...
    do i = 1, rank_A1
      do j = 1, rank_A2
        call check_float_dp(error, actual(i, j), expected(i, j), message, more, thr, rel)
        if (allocated(error)) then
          error%message = error%message // skip // "array mismatch at element index ("//&
            trim(to_string(i))//", "//trim(to_string(j))//")."
          return
        end if
      end do
    end do
  
  end subroutine check_array_dp_dp
